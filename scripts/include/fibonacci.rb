...
private
  ...
  def Fibonacci.run_gcc(size, opt)
    `perl -pi -e 's/const long N = \\d+/const long N = #{size}/' #{CXX_FILE}`
    `#{GCC} -O#{opt} #{CXX_FILE}`
    Common.run_benchmark("fibonacci N=#{size}", "gcc -O#{opt}") do
  ...
  def Fibonacci.run_clang(size, opt)
    ...
    Common.run_benchmark("fibonacci N=#{size}", "clang -O#{opt}") do
      $pid = Process.spawn("./#{CC_OUT}", :out => BENCHMARK_OUT)
      $pid
    end
  end
  ...
  def Fibonacci.run_flix_compiled(size)
    ...
    Common.run_benchmark("fibonacci N=#{size}", "Flix compiled") do
  ...
  def Fibonacci.run_flix_interpreted(size)
    `perl -pi -e 's/Int64 = \\d+i64/Int64 = #{size}i64/' #{FLIX_FILE}`
    Common.run_benchmark("fibonacci N=#{size}", "Flix interpreted") do
  ...
  def Fibonacci.run_java(size)
    ...
    `#{JAVAC} #{JAVA_FILE}`
    Common.run_benchmark("fibonacci N=#{size}", "Java") do
      $pid = Process.spawn("#{JAVA} fibonacci", :out => BENCHMARK_OUT)
      $pid
    end
  end
  ...
  def Fibonacci.run_ruby(size)
    ...
    Common.run_benchmark("fibonacci N=#{size}", "Ruby") do
  ...
  def Fibonacci.run_scala(size)
    ...
    Common.run_benchmark("fibonacci N=#{size}", "Scala") do
